...
and without requiring nuisance convergence rates for validity. Formally, our approach avoids imposing variance stabilization $V_{\mathcal{T}}^2/n_{\mathrm{eff}}\pto V$ (Assumption~\ref{ass:stab}); when this ratio remains random, fixed-variance Wald approximations can fail, motivating self-normalization (Section~\ref{sec:simulation}). This contrasts with
...
\item Sensitivity: vary the forward-block partition (e.g.\ $K$ and burn-in size $|I_1|$) and verify $(\widehat\theta,\widehat V)$ stability; do not post-hoc truncate $1/\pi_t$ beyond the executed overlap rule.
\end{enumerate}
