...
difference sequence, yielding finite-sample conditional unbiasedness.

For inference we studentize by the realized quadratic variation (the sample
variance of the score path) and apply a self-normalized martingale CLT.
This yields asymptotically valid Wald confidence intervals without requiring
(i) stabilization of the adaptive assignment rule $\pi_t\to\pi$, or (ii) any
nuisance convergence rate for \emph{validity} (only predictability, overlap, and
moment/variance-growth conditions). Under $L^2$ nuisance consistency we obtain
oracle equivalence and show nuisance error can only inflate variance.
...
Under logged predictable propensities, design-stage overlap, and mild moment/variance-growth conditions (Sections~\ref{sec:setup}--\ref{sec:studentized_clt}), using \emph{predictable} nuisance fits makes the AIPW/DML score increments an exact martingale difference array; studentizing by the realized quadratic variation (equivalently, the sample variance of the score path) yields fixed-horizon Wald inference for the ATE via a self-normalized martingale CLT. This does \emph{not} require propensity stabilization ($\pi_t\to\pi$) nor nuisance convergence rates for \emph{validity}; nuisance learning enters only through precision/oracle comparisons.
...
We make three contributions. First, we show that under sequential randomization and predictable nuisance construction, the orthogonal AIPW/DML increment is an \emph{exact} martingale difference (Lemma~\ref{lem:mds}), yielding finite-sample unbiasedness. Second, studentizing by realized quadratic variation and applying a self-normalized martingale CLT yields fixed-horizon Wald intervals without requiring propensity stabilization or nuisance convergence rates for \emph{validity} (Theorem~\ref{thm:studentized}). Third, we provide optional precision refinements: a conditional variance decomposition implying nuisance error can only inflate variance (Proposition~\ref{prop:var_decomp}), oracle equivalence under $L^2$ nuisance consistency (Theorem~\ref{thm:oracle_equiv}), and a deterministic-variance CLT under variance stabilization (Corollary~\ref{cor:nonstudentized}).
...
and without requiring nuisance convergence rates for validity. This contrasts with
...
See also inference for batched bandits and related sequential-decision asymptotics \citep{zhang2020inference,hiranoPorter2023asymptotics,liOwen2024batch}.
...
$\cF_t$ & $\sigma(Z_1,\dots,Z_t)$ (post-outcome history) \\
$\cG_t$ & $\sigma(\cF_{t-1},X_t,\pi_t)$ (pre-treatment info) \\
$\pi_t$ & logged propensity; $\Prob(A_t=1\mid \cG_t)=\pi_t$ \\
$m_a^\star$ & true regression $\E[Y(a)\mid X=\cdot]$ \\
$\widehat m_{t-1,a}$ & predictable nuisance used at time $t$ \\
$\phi_t(\cdot)$ & AIPW pseudo-outcome \eqref{eq:phi_def} \\
$\theta_0$, $\widehat\theta$ & ATE \eqref{eq:theta0}; estimator \eqref{eq:theta_hat} \\
$\mathcal{T}$, $n_{\mathrm{eff}}$ & scored index set; $|\mathcal{T}|$ \\
$Q_{\mathcal{T}}$, $V_{\mathcal{T}}^2$ & realized/predictable quadratic variation \eqref{eq:quadratic_variations} \\
$\widehat V$ & sample-variance studentizer \eqref{eq:Vhat_def} \\
\bottomrule
\end{tabular}
...
\midrule
\ref{ass:iid} & i.i.d.\ arrivals of $(X_t,Y_t(0),Y_t(1))$ & Prop.~\ref{prop:identification}, Lem.~\ref{lem:mds}, Thm.~\ref{thm:studentized} \\
\ref{ass:unconf} & sequential randomization with logged $\pi_t$ & Prop.~\ref{prop:identification}, Lem.~\ref{lem:mds}, Prop.~\ref{prop:var_decomp} \\
\ref{ass:overlap} & executed overlap $\pi_t\in[\varepsilon,1-\varepsilon]$ & Lem.~\ref{lem:moment_bounds}, Thm.~\ref{thm:oracle_equiv} \\
\ref{ass:moments} & $\E|Y(a)|^4<\infty$ & Lem.~\ref{lem:moment_bounds}, Thm.~\ref{thm:studentized} \\
\ref{ass:nuis_stab} & uniform 4th moments of $\widehat m_{t-1,a}(X_t)$ & Lem.~\ref{lem:moment_bounds} \\
\ref{ass:predictable_nuis} & nuisances use only past data ($\cF_{t-1}$) & Lem.~\ref{lem:mds}, Thm.~\ref{thm:studentized} \\
\ref{ass:var_growth} & quadratic variation grows ($V_{\mathcal{T}}^2\gtrsim n_{\mathrm{eff}}$) & Thm.~\ref{thm:studentized} \\
\ref{ass:stab} & variance stabilizes ($V_{\mathcal{T}}^2/n_{\mathrm{eff}}\to V$) & Cor.~\ref{cor:nonstudentized} \\
\ref{ass:l2} & $L^2$ consistency of nuisances on scored indices & Thm.~\ref{thm:oracle_equiv} \\
\ref{ass:noise} & nontrivial outcome noise (primitive for var growth) & Lem.~\ref{lem:var_growth_suff} \\
\bottomrule
\end{tabular}
\caption{Assumption ledger (some assumptions are introduced later in the text; this table provides a referee-facing map).}
\label{tab:assumption_ledger}
\end{table}

\begin{remark}[How to enforce Assumption~\ref{ass:nuis_stab}]\label{rem:enforce_nuis}
If outcomes have a natural bounded range (e.g.\ rates, bounded utilities), then
clipping nuisance predictions to that range yields uniform moment bounds while
preserving predictability and martingale unbiasedness (Lemma~\ref{lem:mds}).
\end{remark}
...
All tuning and model selection steps that affect $\widehat m_{t-1}$ are also $\mathcal{F}_{t-1}$-measurable.
\end{assumption}
...
Under forward cross-fitting with a pre-specified burn-in block $I_1$, the scored set $\mathcal{T}=\{1,\dots,n\}\setminus I_1$ is fixed prior to data collection, hence deterministic. This is the setting covered by Theorem~\ref{thm:studentized}. Predictable data-dependent burn-in/scoring rules can be handled with minor extensions (not pursued here).
\end{remark}
...
Let $\widehat{\phi}_t=\phi_t(\widehat m_{t-1})$ with $\widehat m_{t-1}$
$\cF_{t-1}$-measurable. Then there exists $C<\infty$ such that
\[
\sup_{t\le n}\E\big[|\widehat{\phi}_t|^4\big]\le C,\qquad
\sup_{t\le n}\E\big[|\xi_t|^4\big]\le C,\qquad
\sup_{t\le n}\E\big[\xi_t^2\big]\le C,
\]
...
$m^*_a(x)=\E[Y(a)\mid X=x]$ denote the true regressions. Define
$b_a(x):=m^*_a(x)-m_a(x)$ and $\sigma_a^2(x):=\Var(Y(a)\mid X=x)$.
Under Assumptions~\ref{ass:iid} and~\ref{ass:unconf}, for each $t$,
\begin{align}
...
Fix $t$ and condition on $\cG_t$. Let $\varepsilon_{t,a}:=Y_t(a)-m_a^*(X_t)$ so that
...
Proposition~\ref{prop:var_decomp} shows that using $m=m^*$ minimizes the conditional second moment of the pseudo-outcome, and any misspecification (or noisy learning) can only \emph{inflate} variance through a nonnegative augmentation term.
...
\paragraph{Proof sketch (one paragraph).}
Predictability implies $\{\xi_t,\mathcal{F}_t\}$ is an MDS (Lemma~\ref{lem:mds}).
Overlap and moment bounds control the increments (Lemma~\ref{lem:moment_bounds}).
We show realized and predictable quadratic variation are asymptotically
equivalent, and then apply a self-normalized martingale CLT to obtain
$S_{\mathcal{T}}/\sqrt{Q_{\mathcal{T}}}\Rightarrow \mathcal{N}(0,1)$.
Finally we replace $Q_{\mathcal{T}}$ with the feasible $n_{\mathrm{eff}}\widehat V$.
...
To show $Q_{\mathcal{T}}/V_{\mathcal{T}}^2\pto 1$, define
$\Delta_t:=\xi_t^2-\E[\xi_t^2\mid \mathcal{F}_{t-1}]$ and
$M_{\mathcal{T}}:=\sum_{t\in\mathcal{T}}\Delta_t
=Q_{\mathcal{T}}-V_{\mathcal{T}}^2$.
Then $\{M_{\mathcal{T}},\mathcal{F}_t\}$ is a martingale.
...
\[
\E[M_{\mathcal{T}}^2]=\sum_{t\in\mathcal{T}}\E[\Delta_t^2]=O(n_{\mathrm{eff}}).
\]
...
\begin{remark}[Consistency is automatic]\label{rem:consistency}
Under the assumptions of Theorem~\ref{thm:studentized},
$\widehat\theta\pto\theta_0$ whenever $n_{\mathrm{eff}}\to\infty$, since
$\sqrt{n_{\mathrm{eff}}}(\widehat\theta-\theta_0)=O_{\Prob}(1)$.
\end{remark}

\begin{remark}[Validity does not require nuisance convergence rates]\label{rem:no_rates}
The studentized CLT in Theorem~\ref{thm:studentized} does not require $\widehat m_{t-1}$ to converge to the true regressions for \emph{validity}.
The key requirement is \emph{predictability} (Assumption~\ref{ass:predictable_nuis})
together with fourth-moment bounds ensuring the score increments are well behaved
and the quadratic variation grows.
Nuisance learning is nonetheless useful: more accurate regressions reduce the conditional variance of the AIPW pseudo-outcome and therefore tighten confidence intervals.
\end{remark}

\begin{remark}[Contrast with stabilization-based CLTs]\label{rem:contrast_stab}
Existing fixed-horizon results for equal-weight adaptive AIPW/DML estimators typically obtain a \emph{non-studentized} CLT under conditions ensuring a deterministic asymptotic variance---such as stabilization of the assignment rule ($\pi_t\to\pi$) and consistency of nuisance components.
Our Theorem~\ref{thm:studentized} instead provides a \emph{studentized} CLT that does not require policy stabilization: the self-normalizer $\widehat V$ adapts to the realized path of propensities and outcomes on the scored set $\mathcal{T}$.
\end{remark}

\begin{remark}[Relation to realized vs.\ predictable variance]\label{rem:QV}
The variance proxy $\widehat V$ is asymptotically equivalent to $Q_{\mathcal{T}}/n_{\mathrm{eff}}$, the realized quadratic variation per scored unit.
Without Assumption~\ref{ass:stab} (below), neither $Q_{\mathcal{T}}/n_{\mathrm{eff}}$ nor $\widehat V$ need converge to a constant; instead, $\widehat V$ serves as a feasible studentizing factor for \eqref{eq:studentized_clt}.
\end{remark}
...
Let $\mstar_a(x)=\E[Y(a)\mid X=x]$.
Then
\[
\frac{1}{n_{\mathrm{eff}}}\sum_{t\in\mathcal{T}}
\E\Big[
\big(\widehat m_{t-1,1}(X_t)-\mstar_1(X_t)\big)^2
+
\big(\widehat m_{t-1,0}(X_t)-\mstar_0(X_t)\big)^2
\Big]\to 0.
\]
\end{assumption}

\begin{theorem}[Oracle equivalence]\label{thm:oracle_equiv}
Assume Assumptions~\ref{ass:unconf}, \ref{ass:overlap}, \ref{ass:predictable_nuis}, and \ref{ass:l2}. Assume also the scored set $\mathcal{T}$ is deterministic (as in Remark~\ref{rem:T_det}). Let $\mstar=(\mstar_0,\mstar_1)$ and define the oracle estimator
\[
\widehat{\theta}^\star := \frac{1}{n_{\mathrm{eff}}}\sum_{t\in\mathcal{T}} \phi_t(\mstar).
\]
Then
\[
...
\[
\phi_t(\widehat m_{t-1})-\phi_t(\mstar)
=
...
Assume Assumptions~\ref{ass:iid}--\ref{ass:noise} and overlap (Assumption~\ref{ass:overlap}).
...
\item \textbf{Overlap at execution:} enforce $\pi_t\in[\varepsilon,1-\varepsilon]$
\emph{before} drawing $A_t$; the logged $\pi_t$ must match the executed value.
\item \textbf{Predictable nuisances:} for each scored $t$, the fitted nuisance
object $\widehat m_{t-1}$ used in $\widehat\phi_t$ is $\mathcal{F}_{t-1}$-measurable
(e.g.\ forward blocks fit on past blocks, then frozen within the scored block).
\item \textbf{Moment control (practical):} clip nuisance predictions to a
conservative outcome range when appropriate to enforce Assumption~\ref{ass:nuis_stab}.
\item \textbf{Scoring set:} use a deterministic scored set
$\mathcal{T}\subseteq\{1,\dots,n\}$ fixed ex ante (e.g.\ all indices outside a
burn-in block $I_1$), with $n_{\mathrm{eff}}\to\infty$.
\end{enumerate}
...
\paragraph{Minimal audit artifacts (referee-facing).}
Persist per scored block: (i) training indices, (ii) fitted nuisance objects,
(iii) preprocessing/tuning choices, and (iv) a reproducible random seed; verify
the nuisance object reused within a scored block is identical across its units.
...
\item Summary table of $\{\pi_t\}_{t\in\mathcal{T}}$ (min, 5\%, 25\%, median, 75\%, 95\%, max) and the overlap threshold $\varepsilon$ used at execution.
\item Effective per-arm counts: $\sum_{t\in\mathcal{T}}A_t$ and
$\sum_{t\in\mathcal{T}}(1-A_t)$.
\item Variance inflation summary: $\{1/(\pi_t(1-\pi_t))\}_{t\in\mathcal{T}}$
(mean/95th percentile).
\item Sensitivity: vary $\varepsilon$ and the forward-block choice and verify
$(\widehat\theta,\widehat V)$ stability.
\end{enumerate}
...
\paragraph{Design.}
We simulate a sequential experiment with logged propensities that switch after a burn-in. For each replication, draw $n\in\{500,2000\}$ units with $n_0=50$ burn-in assignments at $e_{\mathrm{burn}}=0.5$. Using burn-in data, compute the difference-in-means $\widehat\tau_{\mathrm{burn}}$ and set the post-burn-in propensity to $e_\star=0.8$ if $\widehat\tau_{\mathrm{burn}}\ge 0$ and $e_\star=0.2$ otherwise. Outcomes satisfy $Y(0)=\varepsilon_0$, $Y(1)=\tau+\varepsilon_1$ with $\tau=0$, $\varepsilon_0\sim\mathcal{N}(0,1)$, and $\varepsilon_1\sim\mathcal{N}(0,3^2)$; there are no covariates, so the AIPW score reduces to IPW with logged propensities. We run $R=3000$ replications. We compare: (i) \emph{SN}, the self-normalized interval from Theorem~\ref{thm:studentized}; and (ii) \emph{Fixed}, a misspecified fixed-variance interval computed as if propensities stabilized at $0.5$.
...
\bottomrule
\end{tabular}
\caption{Conditional Monte Carlo coverage (two-sided 95\% Wald intervals) by regime for the regime-switching propensity design.}
\label{tab:sim_regime_conditional}
\end{table}

\paragraph{Interpretation.}
Because the post-burn-in propensity is random, the asymptotic variance is random across replications, violating the stabilization condition behind fixed-variance CLTs. The Fixed method under-covers in the low-propensity regime, while the SN method adapts to the realized quadratic variation and delivers near-nominal coverage in both regimes (with longer intervals when propensities are more extreme).
...
\noindent Table~\ref{tab:assumption_ledger} provides a referee-facing ledger of assumptions and where they are used. In addition, each main result states its invoked assumptions explicitly in the theorem/lemma/proposition statement.
